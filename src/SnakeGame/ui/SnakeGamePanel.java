...
import SnakeGame.enums.GameOverType;
import SnakeGame.enums.GameState;
import SnakeGame.model.Barriers;
import SnakeGame.model.Grid;
import SnakeGame.model.Record;
import SnakeGame.model.Snake;
import SnakeGame.model.SnakeGameRecords;
import SnakeGame.model.record.ReadRecord;
import SnakeGame.model.record.WriteRecord;
...
public class SnakeGamePanel extends JPanel {
	...
	//set flag for each square in a 20 * 30 grid
	private int flag[][] = new int[SnakeGameConstant.GRID_COLUMN_NUMBER][SnakeGameConstant.GRID_ROW_NUMBER];
	
	//set color for each square in a 20 * 30 grid
	private Color color[][] = new Color[SnakeGameConstant.GRID_COLUMN_NUMBER][SnakeGameConstant.GRID_ROW_NUMBER];
	...
	private void initSnake() {
		...
		list.add(0, new Grid(5, 1, SnakeGameConstant.SNAKE_HEAD_COLOR));
		snake.setList(list);
	}
	...
	private void judgeGameOver() {
		if (isSnakeHeadTouchEdge() || isSnakeHeadTouchBody() || isSnakeHeadTouchBarrier()) {
			gameState = GameState.OVER;
			writeScore();
			int result = JOptionPane.showConfirmDialog(frame,  SnakeGameConstant.GAME_OVER, SnakeGameConstant.SNAKE_GAME, JOptionPane.YES_NO_OPTION);
	...
	public void paintComponent(Graphics g) {
		...
		g2.draw(new Rectangle2D.Double(SnakeGameConstant.SNAKE_GAME_PANEL_LEFT,
					...
					SnakeGameConstant.SNAKE_GAME_PANEL_BOTTOM));
	...
	private void drawScore(Graphics g) {
		g.drawString("Score: "+score, SnakeGameConstant.SNAKE_GAME_PANEL_RIGHT+20, 200);
	}
	...
	/**
	 * when score reaches ultiples of 50, a barrier is generated by random
	 */
	...
	/**
	 * judge if the new barrier conflicts with snake body and food
	...
	private void generateBarrierByRandom() {
		...
		if (isBarrierAvailable(randomX, randomY)) {
			barrier.getBarriersList().add(new Grid(randomX, randomY, Color.YELLOW));
			
			//set needToGenerateBarrie as false, if a barrier is generate successfully
	...
	private boolean isSnakeHeadTouchEdge() {
		...
		if ((head.getX() >= SnakeGameConstant.GRID_COLUMN_NUMBER) || (head.getX() < 0)) {
			this.gameOverType = GameOverType.TOUCH_EDGE;
			return true;
		}
		if ((head.getY() >= SnakeGameConstant.GRID_ROW_NUMBER) || (head.getY() < 0)) {
			this.gameOverType = GameOverType.TOUCH_EDGE;
			return true;
		}
	...
	public void saveGameDataToFile(File file) {
		...
		try {
			...
			JOptionPane.showConfirmDialog(frame, "Succeed to Save", "Snake Game", JOptionPane.DEFAULT_OPTION);
		}catch (Exception e) {
	...
	public void loadGameDataFromFile(File file) {
		...
		try {
			...
			if (svaedFlag != null && savedColor != null && savedSnake != null
				...
				JOptionPane.showConfirmDialog(frame, "Succeed to Load", "Snake Game", JOptionPane.DEFAULT_OPTION);
			}
		} catch (Exception e) {
	...
	private void writeScore() {
		...
		if (records == null || records.isEmpty() || !records.isFull()
			...
			if (playerName == null || playerName.length() == 0 || playerName.trim().equals("")) {
				playerName = "Nameless";
			}
